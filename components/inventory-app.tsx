...
import { categories, locations, units, type InventoryItem } from "@/lib/seed";
...
export function InventoryApp({
  ...
  useEffect(() => {
    async function load() {
      try {
        const { workspaceId: foundWorkspaceId, userEmail } =
  ...
  const filteredItems = useMemo(() => {
    return items
      ...
      .filter((item) =>
        locationFilter === "All" ? true : item.locationType === locationFilter
      )
      .filter((item) => {
  ...
  const stats = useMemo(() => {
    ...
    const vanUnits = items
      .filter((item) => item.locationType === "Van")
      .reduce((sum, item) => sum + item.quantity, 0);
    const shopUnits = items
      .filter((item) => item.locationType === "Shop")
      .reduce((sum, item) => sum + item.quantity, 0);
      ...
      <AnimatePresence mode="popLayout">
        {filteredItems.length === 0 ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="rounded-[24px] border border-dashed border-slate-200 bg-white/60 p-8 text-center text-sm text-slate-500"
          >
          ...
          filteredItems.map((item) => (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="rounded-[26px] border border-white/70 bg-white/80 p-4 shadow-sm"
            >
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-slate-500">
                    ...
                    <span>
                      Min:{" "}
                      <strong className="text-slate-800">{item.minStock}</strong>
                    </span>
                    <span>
                      Spot:{" "}
                      <strong className="text-slate-800">
                    ...
                    <span>
                      Total:{" "}
                      <strong className="text-slate-800">
                        {currency(item.quantity * item.unitCost)}
                      </strong>
                    </span>
                    <span>
                      Cheapest:{" "}
                      <strong className="text-slate-800">
                        {item.cheapestStore || item.preferredStore || "—"}
                      </strong>
                    </span>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button variant="outline" onClick={() => moveItem(item.id)}>
                  ...
                  <Button
                    variant="outline"
                    onClick={() => simulatePriceCheck(item)}
                  >
        ...
        <div className="relative overflow-hidden rounded-[36px] border border-white/70 bg-white/45 p-3 shadow-[0_25px_90px_rgba(15,23,42,0.08)] backdrop-blur-2xl md:p-5">
          ...
          <div className="relative z-10 space-y-6">
            <motion.div
              ...
              className="grid gap-4 xl:grid-cols-[1.35fr,0.9fr]"
            >
              <SectionCard
                title={workspaceName}
              ...
              <SectionCard
                title="Live App Stack"
                subtitle="What powers the real build."
              >
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                    <p className="font-semibold text-slate-900">Frontend</p>
                    <p className="mt-1 text-sm text-slate-500">
                      Next.js + mobile-first UI + PWA installability
                    </p>
                  </div>
                  <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                    <p className="font-semibold text-slate-900">Database</p>
                    <p className="mt-1 text-sm text-slate-500">
                      Supabase for live sync, auth, and storage
                    </p>
                  </div>
                  <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                  ...
                  <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      Retail APIs + scheduled refresh jobs
                    </p>
                  </div>
                </div>
              </SectionCard>
            ...
            >
              <StatCard
                ...
                value={compactNumber(stats.totalLines)}
                subtitle="Tracked supply types"
                icon={ClipboardList}
              />
              <StatCard
                title="Total Units"
                value={compactNumber(stats.totalUnits)}
                subtitle="All units on hand"
                icon={Boxes}
              />
              <StatCard
                title="Van Units"
                value={compactNumber(stats.vanUnits)}
                subtitle="Ready for jobs"
                icon={Truck}
              />
              <StatCard
                title="Shop Units"
                value={compactNumber(stats.shopUnits)}
                subtitle="Warehouse stock"
                icon={Warehouse}
              />
              <StatCard
                title="Low Stock"
                value={compactNumber(stats.lowStock)}
            ...
            <motion.div
              ...
              className="grid gap-5 xl:grid-cols-[1.55fr,0.9fr]"
            >
              <div className="space-y-5">
                <SectionCard
                  title="App Navigation"
                  subtitle="The starter is organized into business-ready modules."
                >
                ...
                {activeView === "dashboard" && (
                  <SectionCard
                    title="Dashboard"
                    subtitle="The main daily view for owner and crew."
                  >
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-[24px] border border-white/70 bg-white/80 p-4 shadow-sm">
                        <p className="text-sm font-semibold text-slate-900">
                          What the live app will do
                        </p>
                        <div className="mt-3 space-y-3 text-sm text-slate-600">
                          <div className="flex items-start gap-3">
                            <Cloud className="mt-0.5 h-4 w-4 text-blue-600" />
                            Realtime cloud sync between devices
                          </div>
                          <div className="flex items-start gap-3">
                            <Users className="mt-0.5 h-4 w-4 text-blue-600" />
                            Shared logins for owner and crew
                          </div>
                          <div className="flex items-start gap-3">
                            <Bell className="mt-0.5 h-4 w-4 text-blue-600" />
                            Push alerts for low stock and price changes
                          </div>
                          <div className="flex items-start gap-3">
                      ...
                      <div className="rounded-[24px] border border-white/70 bg-white/80 p-4 shadow-sm">
                        ...
                        <div className="mt-3 space-y-3 text-sm text-slate-600">
                          <div className="flex items-start gap-3">
                            ...
                            Premium iOS and Android style
                          </div>
                          <div className="flex items-start gap-3">
                            <Smartphone className="mt-0.5 h-4 w-4 text-violet-600" />
                            Home screen install as a PWA
                          </div>
                          <div className="flex items-start gap-3">
                            <Tablet className="mt-0.5 h-4 w-4 text-violet-600" />
                            Tablet layout for shop use
                          </div>
                          <div className="flex items-start gap-3">
                ...
                {activeView === "inventory" && (
                  <SectionCard
                    title="Inventory"
                    subtitle="Track van and shop supplies with live controls."
                  >
                    {inventoryCards}
                  </SectionCard>
                )}
                ...
                {activeView === "reorder" && (
                  <SectionCard
                    ...
                    subtitle="Items currently under threshold."
                  >
                    <div className="space-y-3">
                ...
                {activeView === "prices" && (
                  <SectionCard
                    ...
                    subtitle="How the real app will handle cheapest-price tracking."
                  >
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-[24px] border border-white/70 bg-white/80 p-4 shadow-sm">
                        <p className="font-semibold text-slate-900">
                        ...
                        <div className="mt-3 space-y-3 text-sm text-slate-600">
                          ...
                          <div>
                            • Price check button for live Google Shopping lookup
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="rounded-[24px] border border-white/70 bg-white/80 p-4 shadow-sm">
                        ...
                        <div className="mt-3 space-y-3 text-sm text-slate-600">
                          ...
                          <div>
                            • Product matching by SKU, UPC, brand, and title
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {activeView === "team" && (
                  <SectionCard
                    title="Team Access"
                    subtitle="Shared owner and crew workflows."
                  >
                    <div className="grid gap-4 lg:grid-cols-2">
                      <div className="rounded-[24px] border border-white/70 bg-white/80 p-4 shadow-sm">
                        ...
                        <div className="mt-3 space-y-2 text-sm text-slate-600">
                          ...
                          <div>
                            • Manager: inventory edits and reorder approval
                          </div>
                          <div>
                            • Crew: quantity updates and supply checkouts
                          </div>
                        </div>
                      </div>
                ...
                {activeView === "settings" && (
                  <SectionCard
                    title="App Settings"
                    subtitle="Production feature flags."
                  >
                    <div className="grid gap-4 lg:grid-cols-2">
                      ...
                      <div className="rounded-[24px] bg-white/80 p-4 shadow-sm">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm text-slate-500">
                              Crew invitations and permissions
                            </p>
                          </div>
                          <Switch
                            checked={teamAccessEnabled}
                            onCheckedChange={setTeamAccessEnabled}
                          />
                        </div>
                      </div>

                      <div className="rounded-[24px] bg-white/80 p-4 shadow-sm lg:col-span-2">
              ...
              <div className="space-y-5">
                <SectionCard
                  title="Owner Panel"
                  subtitle="Quick business controls."
                >
                  <div className="space-y-3">
                    <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                      <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Automatic pricing needs external APIs.
                          </p>
                        </div>
                        <Badge className="rounded-full border-0 bg-slate-100 text-slate-700">
                          {livePricingEnabled ? "Planned On" : "Preview Only"}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Low stock reminders for owner and crew.
                          </p>
                        </div>
                        <Badge className="rounded-full border-0 bg-blue-100 text-blue-700">
                          {pushAlertsEnabled ? "Enabled" : "Disabled"}
                        </Badge>
                      </div>
                    </div>
                  </div>
                ...
                <SectionCard
                  title="Build Notes"
                  subtitle="What is in the starter."
                >
                  <div className="space-y-3 text-sm text-slate-600">
                    <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                      ...
                      <p className="mt-2">
                        Van and shop inventory, reorder list, supply costs,
                        inventory value, premium mobile UI, route structure, and
                        Supabase starter files.
                      </p>
                    </div>

                    <div className="rounded-2xl bg-white/80 p-4 shadow-sm">
                      <p className="font-semibold text-slate-900">Add next</p>
                      <p className="mt-2">
                        Real logins, shared live syncing, push notifications,
                        and automatic price scanning.
                      </p>
                    </div>
                  </div>
                </SectionCard>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: "Van" | "Shop") =>
                    setForm({ ...form, locationType: value })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, preferredStore: e.target.value })
                  }
                />
              </div>
